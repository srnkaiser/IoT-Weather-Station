...
def analyse():
    ...
    att = [PRED_DIR / f for f in ("attention_60min_features.csv",
                                  "attention_720min_calendar.csv")
           if (PRED_DIR / f).exists()] or sorted(
        PRED_DIR.glob("attention_*.csv"))[:2]
    if att:
...
def make_plots():
    ...
    fig.tight_layout()
    fig.savefig(cfg.RESULTS_DIR / "model_comparison.png", dpi=cfg.FIGURE_DPI)
    plt.close(fig)
    ...
    # Two panels, not one per run. Every run that has attention writes a file,
    # and eight of them side by side produced a 25,000-pixel-wide figure that
    # no manuscript can use. These two carry the whole finding: the short
    # window shows the decay, the 12-hour one shows how far it goes.
    preferred = ["attention_60min_features.csv", "attention_720min_calendar.csv"]
    files = [PRED_DIR / f for f in preferred if (PRED_DIR / f).exists()]
    if not files:
        files = sorted(PRED_DIR.glob("attention_*.csv"))[:2]
    if files:
        ...
        for ax, f in zip(axes[0], files):
            ...
            nice = {"60min_features": "60 min history, engineered features",
                    "720min_calendar": "12 h history"}
            key = f.stem.replace("attention_", "")
            ax.set_title(nice.get(key, key), fontsize=10)
            ax.grid(alpha=0.25)
        fig.suptitle("What the attention layer actually looks at", y=1.0)
        fig.tight_layout()
        fig.savefig(cfg.RESULTS_DIR / "attention_weights.png", dpi=cfg.FIGURE_DPI)
        plt.close(fig)
    ...
    if md.exists():
        ...
        if blocks:
            ...
            fig.tight_layout()
            fig.savefig(cfg.RESULTS_DIR / "regime_breakdown.png", dpi=cfg.FIGURE_DPI)
            plt.close(fig)
...
# --------------------------------------------------------------------------
# Stage 5: paper-ready tables
# --------------------------------------------------------------------------
def paper_tables():
    """
    Emit every table the manuscript needs, generated from results/ rather
    than transcribed.

    Transcribing is how a paper ends up quoting a number no script produces
    any more. Each table here is built from the same JSON the experiments
    wrote, so re-running this after a re-run updates the manuscript inputs
    with it.
    """
    import io
    ...
    w("# Tables for the manuscript\n\n")
    w("Generated by `compare_models.py --paper-tables` from the files in "
      "`results/`. Do not edit by hand - re-run it instead.\n\n")

    # --- Table 1: dataset -------------------------------------------------
    df = load_dataset()
    cut = int(len(df) * (1 - cfg.TEST_SIZE))
    raw_path = cfg.DATASET_FILES[cfg.DATASET]
    n_raw = sum(1 for _ in open(raw_path)) - 1 if raw_path.exists() else None

    w("## Table 1 - Dataset description\n\n")
    w("| Property | Value |\n|---|---|\n")
    w(f"| Source | Max Planck Institute for Biogeochemistry, Jena, Germany |\n")
    w(f"| Period | {df.index.min():%Y-%m-%d} to {df.index.max():%Y-%m-%d} "
      f"({(df.index.max() - df.index.min()).days:,} days) |\n")
    w(f"| Temporal resolution | {cfg.STEP_MINUTES} min |\n")
    if n_raw:
        w(f"| Raw observations | {n_raw:,} |\n")
    # Report the whole chain, not one number. Regularising onto a fixed grid
    # ADDS rows where the record has gaps, so "observations after cleaning"
    # being larger than the raw count looks like an error unless each step is
    # named. The supervised sample count is smaller again, because a lagged
    # feature or a target 60 min ahead is unavailable at the series edges -
    # and it is the supervised count, not the row count, that the results
    # tables refer to.
    n_nan = int(df.isna().any(axis=1).sum())
    w(f"| Rows after regularisation onto the {cfg.RESAMPLE} grid | "
      f"{len(df):,} (gaps in the record are inserted as missing) |\n")
    w(f"| Rows containing a missing value | {n_nan:,} |\n")
    try:
        from features import build_supervised, chronological_split
        X, y, per = build_supervised(df)
        Xtr, Xte, ytr, yte, ptr, pte = chronological_split(X, y, per)
        w(f"| **Usable supervised samples** | **{len(X):,}** |\n")
        w(f"| Training samples (first {100*(1-cfg.TEST_SIZE):.0f} %) | "
          f"{len(Xtr):,} ({Xtr.index.min():%Y-%m-%d} to "
          f"{Xtr.index.max():%Y-%m-%d}) |\n")
        w(f"| Test samples (last {100*cfg.TEST_SIZE:.0f} %) | {len(Xte):,} "
          f"({Xte.index.min():%Y-%m-%d} to {Xte.index.max():%Y-%m-%d}) |\n")
    except Exception as e:
        w(f"| Supervised samples | unavailable: {e} |\n")
    w(f"| Split | chronological, never shuffled |\n")
    w(f"| Forecast target | air temperature, {cfg.FORECAST_HORIZON_MIN} min "
      f"ahead |\n\n")

    w("### Measured variables\n\n")
    w("| Variable | Unit | Sensor in the deployed station | Range in the data "
      "| Mean |\n|---|---|---|---|---|\n")
    sensors = {"temperature": ("degC", "DHT22 / BMP180"),
               "humidity": ("%", "DHT22"),
               "pressure": ("hPa", "BMP180")}
    for col, (unit, sensor) in sensors.items():
        if col in df.columns:
            w(f"| {col} | {unit} | {sensor} | {df[col].min():.1f} to "
              f"{df[col].max():.1f} | {df[col].mean():.1f} |\n")
    w("\n")

    w("### Derived features used for modelling\n\n")
    try:
        from features import build_features
        X = build_features(df)
        groups = {
            "lags": [c for c in X.columns if "_lag" in c],
            "tendencies": [c for c in X.columns if "_tend" in c],
            "rolling statistics": [c for c in X.columns
                                   if "_mean" in c or "_std" in c],
            "humidity-derived": [c for c in X.columns if "dewpoint" in c],
            "cyclic time encoding": [c for c in X.columns
                                     if c.startswith(("hour_", "doy_"))],
        }
        named = {c for g in groups.values() for c in g}
        groups["current values"] = [c for c in X.columns if c not in named]
        w("| Group | Count | Members |\n|---|---|---|\n")
        for g, cols in groups.items():
            if cols:
                w(f"| {g} | {len(cols)} | `{'`, `'.join(sorted(cols))}` |\n")
        w(f"| **Total** | **{len(X.columns)}** | |\n\n")
        w(f"Absolute pressure is deliberately excluded: the training site "
          f"averages {df['pressure'].mean():.0f} hPa while the deployed "
          f"station reports about 1013 hPa, so a model trained on absolute "
          f"level would read every live reading as an extreme. Only "
          f"altitude-invariant pressure features are used.\n\n")
    except Exception as e:
        w(f"(feature table unavailable: {e})\n\n")

    # --- Table 2: configurations and results ------------------------------
    rows = []
    for path in sorted(cfg.RESULTS_DIR.glob("bilstm_*.json")):
        d = json.load(open(path))
        rows.append({
            ...
            "inputs": ("engineered" if d.get("engineered_features") else
                       "raw+calendar" if d.get("calendar_channels") else "raw"),
            "history_min": d["seq_minutes"],
            "units": d.get("units", 64),
            "lr": d.get("learning_rate", 1e-3),
            "params": d["parameters"],
            "seed": d.get("seed"),
            "MAE": d["MAE"], "RMSE": d["RMSE"], "R2": d["R2"],
            "skill": d["Skill_vs_Persistence_%"],
        })
    runs = pd.DataFrame(rows)
    key = ["architecture", "inputs", "history_min", "units", "lr", "params"]
    agg = (runs.groupby(key)
           .agg(runs=("MAE", "size"), MAE_mean=("MAE", "mean"),
                MAE_min=("MAE", "min"), MAE_max=("MAE", "max"),
                skill_mean=("skill", "mean"))
           .reset_index().sort_values("MAE_mean"))
    for c in ("MAE_mean", "MAE_min", "MAE_max"):
        agg[c] = agg[c].round(4)
    agg["skill_mean"] = agg.skill_mean.round(2)

    w("## Table 2 - Model configurations and results\n\n")
    w("All sequence models: Adam, batch 256, dropout 0.15, early stopping on "
      "validation MAE, `ReduceLROnPlateau`, maximum 60 epochs, 15 % of the "
      "training portion held out for validation. Scaling statistics from the "
      "training portion only.\n\n")
    w(agg.to_markdown(index=False) + "\n\n")

    base = json.load(open(cfg.RESULTS_DIR / "forecast_metrics.json"))
    w("### Baseline models (deterministic, single run each)\n\n")
    brows = [{"model": k, "MAE": v["MAE"], "RMSE": v["RMSE"], "R2": v["R2"],
              "skill": v.get("Skill_vs_Persistence_%", 0.0)}
             for k, v in base.items()]
    w(pd.DataFrame(brows).sort_values("MAE").to_markdown(index=False) + "\n\n")

    w("### Every individual run\n\n")
    w(runs.sort_values(["inputs", "architecture", "MAE"])
      .to_markdown(index=False) + "\n\n")

    # --- Table 3: statistics ---------------------------------------------
    mc = cfg.RESULTS_DIR / "model_comparison.json"
    if mc.exists():
        d = json.load(open(mc))
        w("## Table 3 - Statistical comparison\n\n")
        w(f"All models scored on the identical {d['n_common']:,} forecasts, "
          "which is fewer than the test samples in Table 1: each sequence "
          "model additionally drops the first `seq_len` rows of the test "
          "portion, and the comparison is restricted to the timestamps every "
          "model can forecast. "
          "aligned on target timestamp. Diebold-Mariano on absolute-error "
          "differentials with a Newey-West (Bartlett) long-run variance over "
          "144 lags (24 h), because forecast errors on a 10-minute series "
          "stay autocorrelated for hours; a test assuming independence would "
          "overstate every result. Confidence intervals are from a "
          "moving-block bootstrap, 2,000 resamples, block length 24 h. "
          "The reference model is the **median** of its three seeds, not the "
          "best.\n\n")
        w(pd.DataFrame(d["accuracy"]).to_markdown(index=False) + "\n\n")
        w("### Diebold-Mariano against the reference\n\n")
        w(pd.DataFrame(d["significance"]).to_markdown(index=False) + "\n\n")

    # --- Table 4: cost ----------------------------------------------------
    w("## Table 4 - Model size and communication cost\n\n")
    w("Payload per client per federation round, float32, one full parameter "
      "vector per round.\n\n")
    w("| Architecture | Parameters | KB per round |\n|---|---|---|\n")
    seen = set()
    for _, r in agg.iterrows():
        if r.architecture in seen:
            continue
        seen.add(r.architecture)
        w(f"| {r.architecture} | {int(r.params):,} | "
          f"{int(r.params) * 4 / 1024:.0f} |\n")
    w("\nGradient boosting cannot be federated by parameter averaging at "
      "all, which is the structural argument for a neural backbone here, "
      "independent of accuracy.\n\n")

    # --- attention --------------------------------------------------------
    for tag in ("720min_calendar", "60min_features"):
        f = PRED_DIR / f"attention_{tag}.csv"
        if f.exists():
            aw = pd.read_csv(f)
            tot = aw.mean_weight.sum()
            w(f"## Attention mass, {tag}\n\n")
            w("| Window | Steps | Share of attention |\n|---|---|---|\n")
            for c in (10, 30, 60, 120):
                m = aw.minutes_before_issue <= c
                if m.any():
                    w(f"| last {c} min | {m.sum()} of {len(aw)} | "
                      f"{100 * aw.mean_weight[m].sum() / tot:.1f} % |\n")
            w("\n")

    path = cfg.RESULTS_DIR / "paper_tables.md"
    path.write_text(out.getvalue())
    print(f"Saved -> {path.relative_to(cfg.PROJECT_ROOT)} "
          f"({len(out.getvalue()):,} characters)")
...
def main():
    ...
    ap.add_argument("--paper-tables", action="store_true",
                    help="emit every table the manuscript needs")
